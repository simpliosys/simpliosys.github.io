...
var Gravatar = {profile_stack: {}, profile_map: {}, overTimeout: false, outTimeout: false, stopOver: false, active_grav: false, active_hash: false, active_id: false, active_grav_clone: false, profile_cb: null, stats_queue: [], throbber: null, has_bg: false, disabled: false, url_prefix: "http://en", disable: function () {
    ...
    if (null != t) {
        ...
        var k = d * 0.25;
        jQuery("#" + f).removeClass("pos-right pos-left").addClass(c).css({top: (q - k) + "px", left: e + "px"});
        var g = (d / 2);
        if (g > r) {
            g = r / 2
        }
    ...
    h.stop().css({opacity: 0}).show().animate({opacity: 1}, 150, "linear", function () {
    ...
    if (!jQuery("#profile-" + f + " .gcard-links").length && !jQuery("#profile-" + f + " .gcard-services").length) {
        jQuery("#profile-" + f + " .grav-rightcol").css({width: "auto"})
    }
    if (!jQuery("#profile-" + f + " .gcard-about").length) {
        jQuery("#profile-" + f + " .grav-leftcol").css({width: "auto"})
    }
    ...
    var d = {"-webkit-box-shadow": "0 0 4px rgba(0,0,0,.4)", "-moz-box-shadow": "0 0 4px rgba(0,0,0,.4)", "box-shadow": "0 0 4px rgba(0,0,0,.4)", "border-width": "2px 2px " + (this.active_grav.height() / 5) + "px 2px", "border-color": "#fff", "border-style": "solid", padding: "0px", margin: "-2px 0 0 -2px"};
    ...
    var c = this.active_grav_clone.css(d).wrap(b).parent().css({position: "absolute", top: f + "px", left: e + "px", "z-index": 15, border: "none", "text-decoration": "none"});
    ...
    Gravatar.throbber.css({top: b.top + 2 + "px", left: b.left + 1 + "px"})
    ...
    jQuery('img[src*="gravatar.com/"]').addClass("grav-whee").css({"-webkit-box-shadow": "1px 1px 3px #aaa", "-moz-box-shadow": "1px 1px 3px #aaa", "box-shadow": "1px 1px 3px #aaa", border: "2px white solid"});
    var b = 0;
    setInterval(function () {
        jQuery(".grav-whee").css({"-webkit-transform": "rotate(-" + b + "deg) scale(1.3)", "-moz-transform": "rotate(-" + b + "deg) scale(1.3)", transform: "rotate(-" + b + "deg) scale(1.3)"});
        b++;
        if (360 == b) {
            b = 0
        }
...
var GProfile = {data: {}, init: function (b) {
    if ("fetching" == b) {
        return false
    }
