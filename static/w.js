    ...
    var _clickTracker = (function () {
        ...
        var _processLink = function (a, d) {
            try {
                ...
                window._stq.push(['click', {u: a.href, r: ('undefined' !== typeof a.rel) ? a.rel : '0', b: ('undefined' !== typeof _blog) ? _blog : '0', p: ('undefined' !== typeof _post) ? _post : '0'}]);
                if (d) {
                    var now = new Date();
...
window.wpcom.tracks = (function () {
    var userId, userIdType, userLogin, localCache = {}, context = {}, pixel = '//pixel.wp.com/t.gif', cookieDomain = null, cookiePrefix = 'tk_', testCookie = 'tc', userNameCookie = 'ni', userAnonCookie = 'ai', queriesCookie = 'qs', queriesTTL = 1800;
    var TKQ = function (q) {
        this.a = 1;
        if (q && q.length) {
            for (var i = 0; i < q.length; i++) {
    ...
    var newAnonId = function () {
        var randomBytesLength = 18, randomBytes = [];
    ...
    var getCookie = function (key) {
        var name = cookiePrefix + encodeURIComponent(key).replace(/[\-\.\+\*]/g, "\\$&"), pattern = new RegExp("(?:(?:^|.*;)\\s*" + name + "\\s*\\=\\s*([^;]*).*$)|^.*$");
    ...
    var getCookieDomain = function () {
        if (cookieDomain == null) {
            cookieDomain = '';
            var host = document.location.host.toLowerCase().split(':')[0], tokens = host.split('.'), tryDomain;
    ...
    var setCookie = function (key, value, seconds) {
        var name = cookiePrefix + encodeURIComponent(key), date = new Date();
    ...
    var recordEvent = function (eventName, eventProps) {
        _kmq.push(['record', eventName, clone(eventProps)]);
    ...
    var identifyUser = function (newUserId, newUserLogin) {
        if (newUserLogin) {
            _kmq.push(['identify', newUserLogin]);
        ...
        if (anonId) {
            send({_en: '_aliasUser', anonId: anonId});
        }
    ...
    var setProperties = function (properties) {
        _kmq.push(['set', clone(properties)])
    ...
    var API = {storeContext: storeContext, identifyUser: identifyUser, recordEvent: recordEvent, setProperties: setProperties, clearIdentity: clearIdentity};
    initQueue();
    return API;
})();